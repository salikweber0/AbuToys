...
// =================== FIXED GOOGLE SHEETS PRODUCT LOADING ===================

async function loadProductsFromSheet() {
    const sheetURL = "https://opensheet.elk.sh/1LaF1JFdRSGhNvonfo8-G3mHEXIA5-ulPl_4JlIAOAfc/AbuToys";
    ...
    grid.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
            ...
            <p style="font-size: 1.1rem; color: #666;">Loading amazing toys...</p>
        </div>
    `;

    try {
        console.log("Fetching products from:", sheetURL);
        ...
        if (!products || products.length === 0) {
            grid.innerHTML = `
                <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                    <div style="font-size: 2rem; margin-bottom: 15px;">😔</div>
                    <p style="font-size: 1.1rem; color: #666;">No products found in the sheet</p>
                </div>
            `;
            return;
        }
        ...
        products.forEach((product, index) => {
            ...
            // Your sheet structure fix - check multiple possible formats
            const isActive = product["🧸 AbuToys"] === "✅" || 
                            product["Active"] === "✅" || 
                            product["active"] === "✅";
            
            // Skip empty rows or inactive products
            if (!isActive || !product["ProductName"]) {
                console.log(`Skipping product:`, product);
                return;
            }

            // Get product data based on your sheet structure
            const productName = product["ProductName"] || "Unnamed Product";
            const productDesc = product["Desc"] || "No description available";
            const productPrice = product["Price"] || "0";
            const productOldPrice = product["Old Price"] || "";
            const category = product["undefined"] || product["Cat"] || "all"; // your sheet has "undefined" for category
            
            // Images with fallbacks
            const img1 = product["Img1"] || "https://via.placeholder.com/300x300?text=No+Image";
            const img2 = product["Img2"] || img1;
            const img3 = product["Img3"] || img1;

            console.log(`Creating card for: ${productName}`);

            // Create product card
            const productCard = document.createElement("div");
            productCard.classList.add("product-card");
            productCard.setAttribute("data-category", category.toLowerCase());

            productCard.innerHTML = `
                <div class="product-image-container">
                    <div class="product-images">
                        <img src="${img1}" alt="${productName} 1" class="product-img active" 
                             onerror="this.src='https://via.placeholder.com/300x300?text=Image+Not+Found'">
                        <img src="${img2}" alt="${productName} 2" class="product-img"
                             onerror="this.src='https://via.placeholder.com/300x300?text=Image+Not+Found'">
                        <img src="${img3}" alt="${productName} 3" class="product-img"
                             onerror="this.src='https://via.placeholder.com/300x300?text=Image+Not+Found'">
                    </div>
                    <button class="img-nav prev-img"><i class="fas fa-chevron-left"></i></button>
                    <button class="img-nav next-img"><i class="fas fa-chevron-right"></i></button>
                ...
                <div class="product-info">
                    ...
                    <button class="btn btn-small order-btn" data-product-name="${productName}">
                        Order Now
                    </button>
                </div>
            `;
        ...
        if (productCount === 0) {
            grid.innerHTML = `
                <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                    <div style="font-size: 2rem; margin-bottom: 15px;">📦</div>
                    <p style="font-size: 1.1rem; color: #666;">No active products found</p>
                </div>
            `;
        } else {
            // Re-initialize all product-related functionality after loading
            setTimeout(() => {
                initializeProductImageSliders();
                initializeWishlistFunctionality();
                initializeOrderButtons();
                console.log("✅ Product features re-initialized");
            }, 100);
        }
        ...
        grid.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                ...
                <button onclick="loadProductsFromSheet()" style="
                    padding: 10px 20px; margin-top: 15px; background: #FF6B6B;
...
// =================== IMPROVED ORDER FUNCTION ===================
function orderProduct(productName) {
    console.log(`Order function called for: ${productName}`);
    executeAction('whatsapp', productName);
}

// =================== IMPROVED ORDER BUTTONS INITIALIZATION ===================
function initializeOrderButtons() {
    ...
    document.querySelectorAll('.order-btn').forEach(btn => {
        const productName = btn.getAttribute('data-product-name') || 
                           btn.closest('.product-card')?.querySelector('h3')?.textContent?.trim() || 
                           'Product';
    ...
    console.log(`✅ Order buttons initialized for ${document.querySelectorAll('.order-btn').length} products`);
}

// =================== ENHANCED INITIALIZATION ===================
document.addEventListener('DOMContentLoaded', function() {
    console.log('🧸 Starting enhanced product loading...');
    
    // Load products immediately
    loadProductsFromSheet();
    
    // Also try to load after a short delay in case DOM isn't fully ready
    setTimeout(() => {
...
window.testProductLoading = function() {
    ...
    console.log('Current grid content:', document.getElementById('products-grid')?.innerHTML);
    loadProductsFromSheet();
};

console.log('✅ Enhanced product loading system ready!');
